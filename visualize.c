...
static int InitializePackage(SDL_AudioSpec* wavSpec, Uint8* wavStart, Uint32 wavLength, Visualizer_Pkg_ptr vis_pkg)
{
	int ret=0;
...
static void InitializeVariables(struct Visualizer_Pkg *vis_pkg, SDL_AudioSpec have, SDL_AudioDeviceID device)
{
	SDL_AudioSpec *wavSpec;

	wavSpec=GetSDL_AudioSpec(vis_pkg);
    Verify_Audio_Spec(&wavSpec, have);
	vis_pkg->bitsize=SDL_AUDIO_BITSIZE(wavSpec->format);
	...
	vis_pkg->fftw_ptr=allocate_FFTWop(wavSpec->channels, wavSpec->samples);
    //EXIT_IF(ret==-1, device);

	printf("WAV channels %u\n", wavSpec->channels);
	printf("WAV freq: %i\n", wavSpec->freq);
	printf("WAV samples: %i\n", wavSpec->samples);
...
usage:          printf("usage %s [-f] \'PATH/TO/FILE\']\n",argv[0]);
	...
	device=SDL_OpenAudioDevice(NULL, 0, &wavSpec, &have, SDL_AUDIO_ALLOW_ANY_CHANGE);
